...
static void hall_close_sendcfg(struct work_struct *work);
static DECLARE_WORK(hall_close_work, hall_close_sendcfg);
#endif
...
#if KPD_HAS_SLIDE_QWERTY
extern void tpd_enable_hallsensor_dov(int enable);

static void hall_close_sendcfg(struct work_struct *work)
{
    printk("[hall_close_sendcfg]:kpd_slide_state=%d\n",!kpd_slide_state);
    tpd_enable_hallsensor_dov(!kpd_slide_state);
}
...
static void kpd_slide_handler(unsigned long data)
{
	...
	if(tpd_load_status == 1)//TP detected
       {  
            schedule_work(&hall_close_work);
       }
}
