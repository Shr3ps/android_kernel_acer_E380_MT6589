...
u16 Hall_AF_convert(u16 position)// 16bit complement to 10 bit
{

	if(position>0x7FFF)
	{
		position=position+1-0x8001;
		position=position*(1023/2)/((0x7fff+(0xFFFF-0x8001))/2);
	}
	else
	{
		position=position*(1023/2)/((0x7fff+(0xFFFF-0x8001))/2)+(1023/2);
	}
...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	*/
    ...
    { 
		...
		#if 1
    		AFInf=Hall_AF_convert(AFInf);
	    	AFMacro=Hall_AF_convert(AFMacro);
                pCamCalData->Single2A.S2aAf[1] = AFInf-10;
                pCamCalData->Single2A.S2aAf[0] = AFMacro+10;
		#else
