 ...
 * CONTAINED IN MEDIATEK SOFTWARE. MEDIATEK SHALL ALSO NOT BE RESPONSIBLE FOR ANY MEDIATEK2075
...
#define MC3210_DATA_LEN       6// 6  //yanfang 20120521
#define MC3210_DEV_NAME        "MC3210"
#define MC3410_PRODUCT_CODE        0XA8
#define MC3430_PRODUCT_CODE        0X29
#define MC3430_PRODUCT_CODE2        0X39
/*----------------------------------------------------------------------------*/
...
#define cm3410_test   //amy0612 

/* Add for auto detect feature */
static int  mc3210_local_init(void);
static int mc3210_remove(void);
static int mc3210_init_flag =0;
static struct sensor_init_info mc3210_init_info = {		
	.name = "mc3210",		
	.init = mc3210_local_init,		
	.uninit = mc3210_remove,	
};
...
static MC3210_TYPE mc3210_type_flag = NONE_TYPE; //   IS_MC3430 ; //amy0612 NONE_TYPE;
...
static struct data_resolution mc3210_data_resolution[] = {
 /*8 combination by {FULL_RES,RANGE}*/
    ...
    {{ 10, 7}, 86},   /*+/-1.5g  in 8-bit resolution:  15.6 mg/LSB*/  //yanfang 20120521
};
/*----------------------------------------------------------------------------*/
...
static int MC3210_SetDataResolution(struct mc3210_i2c_data *obj)
{
	...
	/*the data_reso is combined by 2 bits: {FULL_RES, DATA_RANGE}*/
		reso = (dat & MC3210_RANGE_MASK)>>2;   //amy0612 remove 
	}
	else 
#else
		if((err = hwmsen_read_byte(obj->client, MC3210_REG_DATA_FORMAT, &dat)))
...
#endif
		if(mc3210_type_flag == IS_MC3430)
	{	
	reso=4;  //yanfang 20120521
	}
	else
	{	
	//	GSE_LOG("mc3210 type error!\n");
	}
...
static int MC3210_ReadData(struct i2c_client *client, s16 data[MC3210_AXES_NUM])
{
	...
	else
	{
		...
		if(mc3210_type_flag == IS_MC3410)
		{
			if((err = hwmsen_read_block(client, MC3210_REG_DATAX0, buf, 0x06)))
			{
				GSE_ERR("error: %d\n", err);
			}
			...
			if(mc3210_type_flag == IS_MC3430)
		{
			if((err = hwmsen_read_block(client, MC3210_REG_DATAX0, buf, 0x03)))
			{
				GSE_ERR("error: %d\n", err);
			}
		    data[MC3210_AXIS_X] = (s8)(buf[MC3210_AXIS_X]);
			data[MC3210_AXIS_Y] = (s8)(buf[MC3210_AXIS_Y]);
			data[MC3210_AXIS_Z] = (s8)(buf[MC3210_AXIS_Z]);
		}
	//	else
		{
	//		GSE_LOG("mc3210 type error\n");
		}
		GSE_LOG("raw data [%5d %5d %5d]\n", data[MC3210_AXIS_X], data[MC3210_AXIS_Y], data[MC3210_AXIS_Z]);
		 /*
		for(i=0;i<3;i++)				
...
static int MC3210_ReadCalibration(struct i2c_client *client, int dat[MC3210_AXES_NUM])
{
    ...
    int mul;
	  ...
	  else
	  {
		  ...
		  if(mc3210_type_flag == IS_MC3410)
		  {
				if((err = hwmsen_read_block(client, MC3210_REG_EXT_DATAX0, buf, 0x06)))
				{
					GSE_ERR("error: %d\n", err);
				}
				data[MC3210_AXIS_X] = (s16)((buf[MC3210_AXIS_X*2]) |
...
static int MC3210_CheckDeviceID(struct i2c_client *client)
{
	...
	if(res <= 0)
	{
        printk("MC3210_CheckDeviceID fail 111!\n");
	...
	if(res <= 0)
	{
        printk("MC3210_CheckDeviceID fail 222!\n");
		goto exit_MC3210_CheckDeviceID;
	}
	mutex_unlock(&obj->i2c_mutex);
...
#ifdef cm3410_test		
	if((res = hwmsen_read_byte(obj->client, MC3210_PRODUCT_CODE_REG, &dat)))
	{
		GSE_ERR("read product code fail!!\n");
		return res;
	}
	//dat=  MC3430_PRODUCT_CODE; //amy0612 add 
	if (dat == MC3410_PRODUCT_CODE)
	{
			mc3210_type_flag = IS_MC3410;
	}
	else if ((dat == MC3430_PRODUCT_CODE)||(dat == MC3430_PRODUCT_CODE2))
	{
			mc3210_type_flag = IS_MC3430;
	}
	else
	{	
	     //  mc3210_type_flag = IS_MC3430; //amy0612 
		printk("[gsensor]PRODUCT_CODE=%x,mc3210 type error!!!!!!\n",dat);
	}
#endif	
#if 0  //yanfang 20120521
...
#else  /*auto detect*/
int mc3210_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
#endif
{
	int err = 0;
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = MC3210_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
...
static int mc3210_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#else  /*auto detect*/
	if(err = mc3210_create_attr(&mc3210_init_info.platform_diver_addr->driver))
#endif
	{
...
#else /*auto detect*/
    sobj.sensor_operate = mc3210_gsensor_operate;
#endif
	...
	kfree(obj);
	exit:
	mc3210_init_flag = -1;  /* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);        
	return err;
}
...
static int mc3210_i2c_remove(struct i2c_client *client)
{
	...
	#else  /*auto detect*/
		if(err = mc3210_delete_attr(&mc3210_init_info.platform_diver_addr->driver))
	#endif
	{
...
static int  mc3210_local_init(void)
{
	...
	if(i2c_add_driver(&mc3210_i2c_driver))
	{
		GSE_ERR("add driver error  mc3210_local_init\n");
		return -1;
	} 
	if(-1 == mc3210_init_flag)	
	{	   
		return -1;	
	}
	return 0;
...
static int __init mc3210_init(void)
{
	...
	#else
	hwmsen_gsensor_add(&mc3210_init_info);
	#endif
	return 0;    
}
/*----------------------------------------------------------------------------*/
