...
#if defined(ACER_C17)
extern void get_tp_fw_id(unsigned char *data);
#endif
...
/* For tp gesture enable/disable */
static int tpd_ges_enable = 0;
static void tpd_ges_en(int en)
{
	if(en)
		tpd_ges_enable=1;
	else
...
int tpd_chk_ges_en(void)
{
	return tpd_ges_enable;
}
EXPORT_SYMBOL(tpd_chk_ges_en);

static ssize_t tpd_store_ges_en(struct device_driver *ddri, const char *buf, size_t count)
{
	int val=0x55;
	if (sscanf(buf, "%d", &val) != 1) {
			TPD_DMESG("%s : Invalid values,buf[0]=%x\n",__func__,buf[0]);
			return -EINVAL;
	}
	...
	TPD_DMESG("enter %s, %d, tpd_ges_enable=%d\n", __FUNCTION__, __LINE__,tpd_ges_enable); 
	return count;
}

static ssize_t tpd_show_ges_en(struct device_driver *ddri, char *buf)
{
	ssize_t res;
...
static DRIVER_ATTR(tpd_ges_en,	0664,	tpd_show_ges_en, tpd_store_ges_en);

static struct driver_attribute *tpd_attr_list[] = {
	&driver_attr_tpd_ges_en,
};
...
{
	int idx, err = 0;
	int num = (int)(sizeof(tpd_attr_list)/sizeof(tpd_attr_list[0]));
	...
	{
		if((err = driver_create_file(driver, tpd_attr_list[idx])))
		{            
			TPD_DMESG("driver_create_file (%s) = %d\n", tpd_attr_list[idx]->attr.name, err);
			break;
		}
	}    
	return err;
...
static int tpd_delete_attr(struct device_driver *driver)
	...
	for (idx = 0; idx < num; idx++) 
	{
		driver_remove_file(driver, tpd_attr_list[idx]);
	}
...
#ifdef TP_GESTURE_SUPPORT
	if((err = tpd_create_attr(&tpd_driver.driver)))
	{
		TPD_DMESG("create attr file fail\n");
		tpd_delete_attr(&tpd_driver.driver);
		return err;
	}
		TPD_DMESG("create attr file success \n");
#endif
