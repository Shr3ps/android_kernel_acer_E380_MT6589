...
//#if defined(ACER_C17)
//extern int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div);
//extern unsigned int bl_div;
...
{
	printk("FL_Sub_Enable\n");
	upmu_set_kpled_sel(0x01);
	upmu_set_kpled_dim_duty(0x1F);
	upmu_set_kpled_en(0x01);
  return 0;
}

int FL_Sub_Disable(void)
{
	printk("FL_Sub_Disable\n");
...
int FL_Sub_duty(kal_uint32 duty)
{
...
enum hrtimer_restart ledTimeOutCallbackSub(struct hrtimer *timer)
{
...
static void timerInit(void)
{
	...
	g_timeOutTimer.function=ledTimeOutCallbackSub;
