...
int cm36283_write_ps_thd(struct i2c_client *client, u8 thd_low, u8 thd_high)
{
	...
	res = CM36283_i2c_master_operate(client, buf, 0x3, I2C_FLAG_WRITE);
	if(res < 0)
	{
	  APS_ERR("i2c_master_send function err\n");
	  return res;
...
/*---------------------------------------------------------------------------------------*/
static ssize_t cm36283_store_en_ps(struct device_driver *ddri, const char *buf, size_t count)
{
	if(!cm36283_obj)
	{
		APS_ERR("cm36283_obj is null!!\n");
...
/*---------------------------------------------------------------------------------------*/
static ssize_t cm36283_store_en_als(struct device_driver *ddri, const char *buf, size_t count)
{
	if(!cm36283_obj)
	{
		APS_ERR("cm36283_obj is null!!\n");
	...
	cm36283_enable_als(cm36283_obj->client,1);
	return count;
}
/*---------------------------------------------------------------------------------------*/
...
static DRIVER_ATTR(enable_als,     S_IWUSR | S_IRUGO,NULL, cm36283_store_en_als);
static DRIVER_ATTR(enable_ps,     S_IWUSR | S_IRUGO, NULL, cm36283_store_en_ps);
/*----------------------------------------------------------------------------*/
...
static int cm36283_init_client(struct i2c_client *client)
{
	...
	if(0 == obj->hw->polling_mode_ps){
        ...
        if((0 == obj->hw->polling_mode_als) || (0 == obj->hw->polling_mode_ps))//LK@add
        {
